                ...
                rows.each(function() {
                    $(this).find("td,th").not(options.ignoreColumns)
                        .each(function(i, col) {
                            ...
                            var fixForExport = 0;
                            if(column.find('.fixForExport').length > 0){ fixForExport = 1; }
...
function exportCSVfile(type, table, module, callback){
    table.tableHTMLExport({
        type: type,
        filename: module + '.' + type,
        ignoreColumns: '.toolColumn, .toolRow',
        ignoreRows:'.ignoreRow, .ignoreRowSelectMultiple',
    });
    setTimeout(function(){if(typeof callback === 'function'){ callback() }}, 100);
}