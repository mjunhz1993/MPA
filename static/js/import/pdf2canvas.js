    ...
    class PDFjsViewer {
        constructor($container, options = {}) {
            let defaults = {
                ...
                onPageRender: (page, i) => {},
                errorPage: () => {
                    $(`<div class="placeholder"></div>`).addClass(this.settings.pageClass).append($(`<p class="alert alertRed"></p>`).text("could not load document"));
                },
                zoomValues: [ .25, .5, .75, 1, 1.25, 1.5, 2, 4, 8 ],
        ...
        RUN(callback){
            this.$container.html(HTML_loader());
            loadJS('https://cdnjs.cloudflare.com/ajax/libs/pdf.js/2.11.338/pdf.min.js', function(){
        ...
        async loadDocument(document){
            remove_HTML_loader(this.$container);
            this.pages = [];
            this.$container.find(`.${this.settings.pageClass}`).remove();
            this.pdf = null;
...
function pdf2canvas(d){
    var pdfViewer = new PDFjsViewer(d.box, {onPageRender: function(page, i){ d.eachPaper(page.find('canvas'), i) }});
    pdfViewer.RUN(function(){ pdfViewer.loadDocument(d.pdf) });
}